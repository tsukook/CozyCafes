...
public class CzCClientGameEventBusSubscriber {
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        if (event.getStage() == RenderLevelStageEvent.Stage.AFTER_CUTOUT_BLOCKS) {
            DandelionSeedRenderer.render(Minecraft.getInstance().renderBuffers().bufferSource().getBuffer(RenderType.translucentParticle(ResourceLocation.fromNamespaceAndPath(CozyCafes.MODID, "textures/atlas/particles.png"))), event.getCamera(), event.getPartialTick().getGameTimeDeltaPartialTick(event.getLevel().tickRateManager().runsNormally()));
        }
    }
